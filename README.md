...
**CI:** o [`.github/workflows/backend-trivy.yml`](.github/workflows/backend-trivy.yml) roda **apenas em pull request para `main`** (`opened`, `synchronize`, `reopened`), com filtro de paths em `backend/` — **não** dispara em push só na branch sem PR, nem no **push em `main`** após o merge. A cada novo commit no PR o scan roda de novo; para **só** a primeira abertura, restrinja no workflow `types` a `[opened]` (aceita o risco de merges sem re-scan no PR). Dois jobs: **filesystem** e **imagem Docker**; severidade **CRITICAL** e **HIGH**, `exit-code: 1` se houver achado. O **deploy** só publica a imagem.
...
4. **🚀 Deploy no Kubernetes**
   - **Branch `main`:** deploy no cluster de **produção** (DigitalOcean), imagens **`:prod`**
   - **Branch `develop`:** deploy no cluster de **desenvolvimento** (Hostinger), imagens **`:develop`**
   - Após `kubectl apply`, o workflow faz **`kubectl set image`** para a tag do ambiente (evita ficar preso em `:latest` sem puxar nova imagem)
...
- ✅ Manual dispatch (`workflow_dispatch`; deploy só nas branches acima)
- ⏭️ Pull requests **não** disparam este workflow (após merge, o push à branch sim)
- ⏭️ Ignora apenas alterações em ficheiros **markdown** (por exemplo `README.md`); mudanças em **`k8s/`** disparam o workflow

#### Configuração de Ambiente

**Frontend (Vite):** ficheiros de ambiente versionados por modo de build
- `frontend/.env.production` — API **produção** (`https://api.gds.propepi.org.br/v1`), usado no build da branch **`main`**
- `frontend/.env.development` — API **dev** (`https://devapi.gds.proepi.org.br/v1`), usado no build da branch **`develop`**
- O Docker build recebe `VITE_BUILD_MODE=production` ou `development` conforme a branch (ver `frontend/Dockerfile`)

#### Tags das Imagens

Cada build gera múltiplas tags, incluindo:
- **`prod`** — última imagem publicada a partir da branch **`main`** (deploy de produção usa esta tag)
- **`develop`** — última imagem a partir da branch **`develop`** (deploy de dev usa esta tag)
- `latest` — na branch padrão do repositório (normalmente `main`)
- `main` / `develop` — nome da branch
...
### 🔐 Configuração de Secrets (kubeconfig na CI)

O deploy usa **dois secrets** (um por cluster):

| Secret | Uso |
|--------|-----|
| **`KUBE_CONFIG`** | Cluster de **desenvolvimento** (push em **`develop`**) |
| **`KUBE_CONFIG_PROD`** | Cluster de **produção** (push em **`main`**) |

**Conteúdo:** o ficheiro kubeconfig completo codificado em **Base64** (uma única linha).
...
No GitHub: **Settings → Secrets and variables → Actions → New repository secret** — crie **`KUBE_CONFIG`** e **`KUBE_CONFIG_PROD`** com esses valores.

**Requisitos para a CI:** o kubeconfig deve apontar para um **endereço API do cluster acessível a partir da internet** (por exemplo `https://<IP ou hostname>:6443`). Um `server: https://127.0.0.1:...` típico de `kubectl proxy` local **não** funcionará nos runners do GitHub. Confirme com:
...
**Sem o secret do ambiente correspondente:** o build e o push para o GHCR continuam a correr; o job de **deploy** desse ambiente falha até o secret estar correto.

**Branchs:** push em **`develop`** faz deploy só no cluster de dev; push em **`main`** só no de produção. Ver também [`.github/workflows/README.md`](.github/workflows/README.md).
