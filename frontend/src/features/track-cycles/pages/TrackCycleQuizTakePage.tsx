...
import {
  useQuizSubmissions,
  useCreateQuizSubmission,
} from "../../quizzes/hooks/useQuizSubmissions";
...
export default function TrackCycleQuizTakePage() {
  const {
    id: cycleIdParam,
    participationId: participationIdParam,
  ...
  const sequenceWithForm = useMemo(() => {
    const track = progress?.track_cycle?.track as
      | { section?: any[] }
      | undefined;
    ...
    for (const section of track.section) {
      ...
      if (seq && seq.form_id) {
        return { sequence: seq, formId: seq.form_id };
      }
    }
  ...
  if (progressError) {
    return (
      <Box sx={{ p: 3 }}>
        <ErrorAlert
          message={
            (progressError as Error)?.message ?? "Erro ao carregar progresso"
          }
        />
        <Button
  ...
  const safeParticipationId = progress.participation_id;
    ...
    return (
      <Box sx={{ p: 3, maxWidth: 1200, mx: "auto" }}>
        ...
        <QuizRenderer
          ...
          participationId={safeParticipationId}
          formVersionId={formVersion.id}
  ...
  return (
    <Box sx={{ p: 3, maxWidth: 1200, mx: "auto" }}>
      ...
      {existingSubmissions.length > 0 && (
        <Alert severity="info" sx={{ mb: 2 }}>
          Já existem {existingSubmissions.length} tentativa(s). Esta será a
          tentativa {existingSubmissions.length + 1}.
        </Alert>
      )}
        ...
        >
          ...
          >
            ...
            <Box sx={{ flex: 1 }}>
              ...
              >
                {quiz.description ??
                  "Leia atentamente as questões e responda com cuidado. Boa sorte!"}
              </Typography>
            </Box>
          </Box>
