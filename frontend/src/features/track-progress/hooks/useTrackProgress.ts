...
export function useStartTrackProgress() {
  ...
  return useMutation({
    mutationFn: (data: StartTrackProgressDto) =>
      TrackProgressService.start(data),
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({
        queryKey: ["track-cycles", variables.trackCycleId, "students"],
      });
...
export function useCanAccessSequence(
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await TrackProgressService.canAccessSequence(
        trackProgressId,
        sequenceId,
      );
      return response.data;
...
export function useUpdateSequenceProgress(
  ...
  return useMutation({
    mutationFn: (data: UpdateSequenceProgressDto) =>
      TrackProgressService.updateSequenceProgress(
        trackProgressId,
        sequenceId,
        data,
      ),
...
export function useCompleteQuiz(trackProgressId: number, sequenceId: number) {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: (quizSubmissionId: number) =>
      TrackProgressService.completeQuiz(
        trackProgressId,
        sequenceId,
        quizSubmissionId,
      ),
